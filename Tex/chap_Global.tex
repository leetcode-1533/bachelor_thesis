...
\center
{
\includegraphics[width=\textwidth]{Img/pca_correlation.png} \captionof{figure}{PCA基底的正交性}
}
\end{minipage}
\medskip
\end{center}
...
\center
{
\includegraphics[width=\textwidth]{Img/nmf_cost.pdf} \captionof{figure}{NMF的惩罚函数下降曲线 \\ x坐标:迭代次数, y坐标 惩罚函数\ref{nmf_cost}}
}
\end{minipage}
\medskip
\end{center}
...
\medskip
\end{center}
\todo{Projected Gradient Methods for Non-negative Matrix Factorization}
...
\center
{
\includegraphics[width=\textwidth]{Img/pni_baseconstr} \captionof{figure}{基底创建时间 \\其中ICA分别用了两种不同的配置}
\label{fig:ica_base}
}
\end{minipage}
\medskip
...
\center
{
\includegraphics[width=\textwidth]{Img/pni_baseproj} \captionof{figure}{基底投影时间}
\label{fig:ica_base}
}
\end{minipage}
\medskip
...
\center
{
\includegraphics[width=\textwidth]{Img/pni_reconstr.png} \captionof{figure}{测试图像的投影\\横轴:左4:PCA, 中4: NMF, 右4: ICA, 纵轴: 维度依次增加}
\label{fig:ica_base}
}
\end{minipage}
\medskip
...
\subsection{\ref{sec:pni_recon}的应用}
实际上\ref{sec:pni_recon}中MSE的区别是可以被加以利用来识别场景的.以下是对应的ROC曲线的绘制:

\subsection{结合SVM的分类实验}
当全局特征提取后,自然地,对这些样本本项目尝试了使用SVM的分类识别的测试.其实验过程:
	\begin{itemize}
		\item 使用k-fold Cross Validation的方法,K取值为3
	...
	\end{itemize}
	经过上述的测试过程后,以下是实验的结果,其中,识别率由z轴表示,从$5-100$变化的轴表示了基底的数量.从$1-9$变化的轴表示了学习的样本比例.
	\begin{center}
	\begin{minipage}[t]{\linewidth}
	%\label{fig:main}
	\center
	{
	\includegraphics[width=\MyFactor\textwidth]{Img/svm_pca} \captionof{figure}{PCA结合SVM的分类实验}
	}
	\end{minipage}
	\medskip
	\end{center}
	...
	\center
	{
	\includegraphics[width=\MyFactor\textwidth]{Img/svm_nmf} \captionof{figure}{NMF结合SVM的分类实验}
	}
	\end{minipage}
	\medskip
	\end{center}
	...
	\center
	{
	\includegraphics[width=\MyFactor\textwidth]{Img/svm_ica} \captionof{figure}{ICA结合SVM的分类实验}
	}
	\end{minipage}
	\medskip
	\end{center}