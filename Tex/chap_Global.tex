...
\end{enumerate}
	...
	\center
	{
	\includegraphics[width=\textwidth]{Img/pni_recon} \captionof{figure}{重构输入样本示意}
	}
	\end{minipage}
	\medskip
	\end{center}
...
\paragraph{识别结果} 当全局特征提取后,自然地,对这些样本本项目尝试了使用SVM的分类识别的测试.其实验过程:
	...
	\end{enumerate}
	SVM使用径向基作为核函数,维度为3,惩罚函数参数为0.5,容错值为0.001.\newline
...
Confusion Matrix是表征分类情况的图像,以下是某次SVM运行的结果,采用了5-fold Validation.综合成功率94\%.
...
上一节详细使用了SVM作为分类机,本届将遍历分类算法,来综合测试.具体过程如下:
	\begin{enumerate}
		\item PCA, NMF, ICA以及在DCT空间的PCA, ICA均从ORL数据库40人的前3张图片计算出该组数据库的长度为64位的基底.
		\item ORL数据库参加基底运算的3张图片和没有参加的7张图片全部被上一步的运算投影到长度为64的坐标
		\item 结合原有的标签,采用\textit{4-folds cross-validation}的方法,进行分类测试,并记录准确率
	\end{enumerate}
